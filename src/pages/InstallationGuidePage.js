...
function InstallationGuidePage() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-5">
      <div id="installation-container" className="max-w-4xl w-full bg-card-bg rounded-xl shadow-xl p-10 border border-border-color mt-5 mb-5 relative transition-all duration-300 ease-in-out hover:shadow-accent-glow hover:translate-y-[-2px]">
        <ProgressBar currentStep={currentStepIndex + 1} totalSteps={totalSteps} />

        <h1 className="text-4xl text-accent-color mb-5 text-center font-bold text-shadow-accent">
          Arch Linux Installation Guide
        </h1>

        <div className="mb-8">
          <h2 className="text-3xl text-text-color mt-5 mb-4 font-semibold border-b-2 border-border-color pb-2">
            Step {currentStepIndex + 1}: {step.title}
          </h2>
          <p className="mb-4 text-text-color opacity-90">{step.description}</p>
        ...
        <div className="flex justify-between mt-10 gap-5">
          <button
            ...
            className="flex-1 bg-button-bg text-text-color border-none py-4 px-6 rounded-lg cursor-pointer text-lg font-semibold transition-all duration-300 ease-in-out shadow-md hover:bg-button-hover-bg hover:translate-y-[-3px] hover:shadow-accent-glow-md disabled:bg-border-color disabled:cursor-not-allowed disabled:opacity-60 disabled:transform-none disabled:shadow-none"
          >
            Previous Step
          </button>
          <button
        ...
        <Link
          to="/"
          className="block text-center mt-8 text-text-color no-underline font-semibold text-base py-3 px-5 rounded-lg bg-button-bg transition-all duration-300 ease-in-out shadow-md max-w-xs mx-auto hover:bg-button-hover-bg hover:translate-y-[-2px] hover:shadow-accent-glow-md"
        >
          Back to Home
        </Link>
      </div>
