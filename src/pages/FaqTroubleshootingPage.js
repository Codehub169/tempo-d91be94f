  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-4">
      <div className="container max-w-4xl w-full bg-card-bg-transparent rounded-xl shadow-xl p-8 md:p-10 my-8 border border-border transform transition-all duration-300 hover:shadow-accent-glow hover:scale-[1.005]">
            ...
            <Accordion key={index} title={item.question}>
              {item.answer.map((contentItem, contentIndex) => {
                if (contentItem.type === 'paragraph') {
                  ...
                  return <CodeBlock key={contentIndex} command={contentItem.code} explanation={contentItem.explanation} />;
        ...
        <p className="mb-4 opacity-90 mt-8">
          For more comprehensive troubleshooting, the{" "}
          <a
            href="https://wiki.archlinux.org/index.php/Category:Troubleshooting"
            target="_blank"
            rel="noopener noreferrer"
            className="text-accent no-underline hover:underline"
          >
            Arch Wiki Troubleshooting category
          </a>{" "}
        ...
        <div className="flex flex-wrap justify-center gap-4 mt-12">
          ...
          <Link
            to="/post-install"
            className="bg-button-bg hover:bg-button-hover-bg text-text font-semibold py-2 px-6 rounded-lg transition duration-300 ease-in-out transform hover:-translate-y-1 shadow-lg hover:shadow-accent-glow-md"
          >
...
export default FaqTroubleshootingPage;